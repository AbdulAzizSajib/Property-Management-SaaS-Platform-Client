...
export default function ReportsPage() {
  ...
  return (
    <div className="min-h-screen bg-cream print:bg-white">
      ...
      <div className="mx-auto container space-y-6 px-4 py-6 sm:px-6 lg:px-8 lg:py-8 print:max-w-full print:p-0 print:space-y-4">
        {/* Heading */}
          ...
          {report && (
            <div className="mt-3 flex flex-wrap items-center justify-between gap-2 border-t border-rule-soft pt-3 text-[12px] text-ink-soft">
              <span className="inline-flex items-center gap-1.5">
                <Calendar size={12} className="text-ink-soft/60" />
...
function MonthlyChart({ monthly }: { monthly: FinancialReportMonthly[] }) {
  const max = useMemo(
    () =>
      Math.max(
        1,
        ...monthly.flatMap((m) => [Number(m.income), Number(m.expense)]),
      ),
    [monthly],
  );
              ...
              <tr key={m.month} className="hover:bg-cream/40">
                ...
                <td className="px-3 py-2 text-right tabular-nums text-jade-800">
                  {incomeVal > 0 ? (
                    <span className="inline-flex items-center gap-1">
                      <ArrowUpRight size={11} className="text-jade-700" />
                      {formatMoney(incomeVal)}
                    </span>
                  ) : (
                    <span className="text-ink-soft/50">{formatMoney(0)}</span>
                  )}
                </td>
                <td className="px-3 py-2 text-right tabular-nums text-coral-600">
                  {expenseVal > 0 ? (
                    <span className="inline-flex items-center gap-1">
                      <ArrowDownRight size={11} className="text-coral-600" />
                ...
                <td
                  className={cn(
                    ...
                    isProfit ? "text-jade-950" : "text-coral-600",
                  )}
                >
                  {isProfit ? "" : "−"}
