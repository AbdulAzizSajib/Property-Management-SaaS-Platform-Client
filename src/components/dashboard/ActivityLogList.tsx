      ...
      <div className="mx-auto container space-y-5 px-4 py-6 sm:px-6 lg:px-8 lg:py-8">
        ...
        {isLoading ? (
          <div className="space-y-2">
            {[1, 2, 3, 4, 5].map((i) => (
              <Skeleton key={i} className="h-14 rounded-[10px] bg-paper" />
            ))}
          </div>
        ) : isError ? (
          ...
          <div className="rounded-[14px] border border-dashed border-rule-soft bg-paper px-6 py-12 text-center">
            <Activity size={28} className="mx-auto text-ink-soft/50" />
    ...
    <li className="rounded-[10px] border border-rule-soft bg-paper">
      ...
      >
        ...
        <div className="min-w-0 flex-1">
          <p className="truncate text-[13px] font-semibold text-ink">
            ...
            <span className="ml-1.5 text-ink-soft">· {log.entityType}</span>
          </p>
          <p className="text-[11px] text-ink-soft">
            {log.user?.name ?? "system"} · {formatTimestamp(log.createdAt)}
          </p>
        </div>
        <ChevronDown
          size={14}
      ...
      {open && (
        <div className="border-t border-rule-soft px-3 py-2 text-[11.5px]">
          {log.description && <p className="text-ink">{log.description}</p>}
          {log.metadata && (
