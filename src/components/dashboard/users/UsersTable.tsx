      ...
      <div className="mx-auto container space-y-5 px-4 py-6 sm:px-6 lg:px-8 lg:py-8">
        ...
        {isLoading ? (
          <div className="space-y-2">
            {[1, 2, 3, 4].map((i) => (
              <Skeleton key={i} className="h-16 rounded-[10px] bg-paper" />
            ))}
          </div>
        ) : isError ? (
          <ErrorBlock
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {toDeactivate?.name} will lose access immediately. This is a soft
              delete — the record stays in the database.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <Button
