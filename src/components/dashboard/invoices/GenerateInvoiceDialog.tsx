...
export function GenerateInvoiceDialog({
  ...
  const previewTotal = selectedLease
    ? Number(selectedLease.monthlyRent) + Number(selectedLease.serviceCharge)
    : 0;
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle className="text-jade-950">Generate invoice</DialogTitle>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <Field label="Lease" htmlFor="g-lease" required>
            ...
            >
              <SelectTrigger id="g-lease" className={`w-full ${fieldClass}`}>
                        ...
                        <span className="inline-flex w-full items-center gap-2">
                          ...
                          <span>
                            ...
                            <span className="ml-1.5 text-ink-soft">
                              · {l.unit.building.name} · {l.unit.name}
                            </span>
                          </span>
                          {leaseDue > 0 && (
          ...
          <Field label="Billing month" htmlFor="g-month" required>
            <Input
              id="g-month"
              type="month"
              value={billingMonth}
                    ...
                    <li key={c.id}>
                      <label className="flex cursor-pointer items-center gap-2.5 rounded-[8px] border border-rule-soft bg-paper px-2.5 py-2 transition-colors hover:border-coral-200">
                        <input
                          type="checkbox"
                          checked={checked}
                          onChange={() => toggleCarry(c.id)}
                          className="size-4 shrink-0 accent-jade-800"
                        />
                        <span className="min-w-0 flex-1">
                          <span className="block text-[12.5px] font-medium text-ink">
                            {formatBillingMonth(c.billingMonth)}
                          </span>
          ...
          {selectedLease ? (
            <div className="rounded-[10px] border border-jade-100 bg-jade-50/60 px-3 py-2.5 text-[12px]">
              ...
              <ul className="mt-1.5 space-y-0.5 text-ink">
                <li className="flex justify-between tabular-nums">
                  <span>Base rent</span>
                  <span>{formatMoney(selectedLease.monthlyRent)}</span>
                </li>
                <li className="flex justify-between tabular-nums">
                  <span>Service charge</span>
                  <span>{formatMoney(selectedLease.serviceCharge)}</span>
                </li>
              </ul>
            ...
            <div className="flex items-start gap-2 rounded-[10px] border-l-[2.5px] border-coral-500 bg-cream/70 px-3 py-2 text-[12.5px] text-ink">
              <Info size={13} className="mt-0.5 shrink-0 text-coral-600" />
              <span>
